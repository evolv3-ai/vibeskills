...
Prerequisites:
- Provider skill installed (e.g., `admin-infra-oci`)
- `admin-app-coolify` installed

Steps:
1. Provision server via the provider skill (2+ vCPU, 8GB+ RAM).
2. Add server to inventory with `ROLE=coolify`.
3. Install Coolify using `admin-app-coolify`.
4. Configure a tunnel if needed (see `admin-app-coolify` references).
5. Update inventory:
...
3. Install services via `admin-app-*` skills.
